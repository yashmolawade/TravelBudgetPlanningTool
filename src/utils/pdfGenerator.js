...
export const generatePDFReport = ({
  ...
  doc.text('Travel Budget Report', pageWidth / 2, yPosition, { align: 'center' })
  yPosition += 15
  ...
  const summaryData = [
    ...
    ['Budget Usage', totalBudget > 0 ? `${Math.round((totalSpent / totalBudget) * 100)}%` : '0%']
  ]
  ...
  const categoryData = categories
    .filter(category => (categoryTotals[category] || 0) > 0 || (budgets[category] || 0) > 0)
    .map(category => {
  ...
  // Save the PDF
  const fileName = `travel-budget-report-${new Date().toISOString().split('T')[0]}.pdf`
  doc.save(fileName)
}
